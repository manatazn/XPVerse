...
date_default_timezone_set('Europe/Moscow'); // Server time consistency (configurable reset)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($settings['maintenance']) && $settings['maintenance'] === true) {
        if ($uid !== $masterAdmin && $uid !== '5461064199') {
            echo json_encode(['error' => 'MAINTENANCE', 'message' => 'The bot is currently undergoing maintenance. Please try again later.']);
            exit;
        }
    }
    ...
    if (!isset($users[$uid])) {
        $users[$uid] = [
            ...
            'boxesOpened' => 0,
            'streak' => 1,
            'lastResetDay' => $today, // Uses dynamic logical day
            'referrer' => null,
            'sponsorAzx' => false,
            'rejectedTasks' => [],
    ...
    if (strpos($action, 'admin_') === 0) {
        ...
        if ($action === 'admin_action_submission') {
            ...
            echo json_encode(['error' => 'Təsdiq tapılmadı.']); exit;
        }
        ...
        if ($action === 'admin_action_withdraw') {
            ...
            if (isset($withdrawals[$targetUid][$idx])) {
                if ($withdrawals[$targetUid][$idx]['status'] === 'Pending') {
                    ...
                    echo json_encode(['error' => 'Artıq icra olunub']); exit;
                }
            } else {
    ...
    switch ($action) {
        ...
        case 'claim_task':
            ...
            if ($taskId === 'sponsor_azx') {
                ...
                if (!in_array($taskId, $tasks[$uid])) {
                    ...
                    // Check if it's a custom task without info requirement
        ...
        case 'sync':
            // Just returns current state without writing active time (handled above)
            break;
    }
  ...
  <script>
    ...
    async function apiCall(action, payload = {}, isSilent = false) {
      try {
        ...
        if(data.error) {
          ...
          if(!isSilent) showToast("Error", data.error, "error");
          return false;
        }
        ...
        if(!isSilent) showToast("Connection Error", "Could not reach server.", "error");
        return false;
      }
    }
    ...
    function renderTasks() {
      ...
      const missionsList = [
        ...
        { id: 'complete_all', label: 'Complete All Tasks', icon: 'fa-check-to-slot', color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20', reward: 100*mult, target: 30, current: appState.user.adsWatchedToday }
      ];
    ...
    function renderCustomTasks() {
        const container = document.getElementById('custom-tasks-container'); container.innerHTML = '';
        if(!appState.custom_tasks) return;
        
        appState.custom_tasks.forEach(ct => {
            const isCompleted = appState.tasks.includes(ct.id);
            const isRejected = (appState.user.rejectedTasks || []).includes(ct.id);
            const isPending = appState.pending_submissions.some(s => s.taskId === ct.id);
            
            let btnHtml = '';
            if (isCompleted) {
                btnHtml = `<span class="text-[9px] font-black bg-emerald-500/10 text-emerald-400 px-2 py-1.5 rounded-lg border border-emerald-500/30 flex items-center gap-1 shadow-inner"><i class="fa-solid fa-check-double"></i> Completed</span>`;
            } else if (isRejected) {
                btnHtml = `<span class="text-[9px] font-black bg-red-500/10 text-red-400 px-2 py-1.5 rounded-lg border border-red-500/30 shadow-inner">Rejected</span>`;
            } else if (isPending) {
                btnHtml = `<span class="text-[9px] font-black bg-amber-500/10 text-amber-400 px-2 py-1.5 rounded-lg border border-amber-500/30 shadow-inner">Gözləmədə</span>`;
            } else {
                if (ct.requireInfo) {
                    btnHtml = `<button onclick="openCustomTaskModal('${ct.id}', '${ct.title.replace(/'/g, "\\'")}', '${ct.infoLabel.replace(/'/g, "\\'")}', '${ct.link}')" class="text-[10px] font-black bg-blue-600 text-white px-3 py-1.5 rounded-lg active:scale-95 transition-all">Start</button>`;
                } else {
                    btnHtml = `<button onclick="claimCustomTaskDirect('${ct.id}', '${ct.link}')" class="text-[10px] font-black bg-blue-600 text-white px-3 py-1.5 rounded-lg active:scale-95 transition-all">Claim</button>`;
                }
            }
            ...
            <div class="glass-card rounded-xl p-3 flex justify-between items-center border border-slate-800 shadow-sm">
                <div class="flex items-center gap-2.5">
                    <div class="w-10 h-10 rounded-lg border bg-slate-800/50 border-slate-700 flex items-center justify-center">
                        <i class="fa-solid fa-link text-slate-400 text-lg"></i>
                    </div>
                    <div class="flex flex-col max-w-[150px]">
                        <span class="text-[13px] font-black text-white tracking-wide truncate">${ct.title}</span>
                        <span class="text-crypto-glow text-[9px] font-bold tracking-widest uppercase mt-0.5">+${ct.reward * appState.settings.xpMultiplier} XP</span>
                    </div>
                </div>
                ${btnHtml}
            </div>`;
    ...
    function openCustomTaskModal(id, title, label, link) {
        ...
        document.getElementById('ct-modal-label').innerText = label || 'Info:';
    ...
    async function submitCustomTaskInfo() {
        const taskId = document.getElementById('ct-modal-id').value;
        const info = document.getElementById('ct-modal-input').value.trim();
        if(!info) return showToast('Xəta', 'Məlumat daxil edin', 'error');
        
        const res = await apiCall('submit_task_info', { taskId, info });
        if(res && !res.error) {
    ...
    async function claimCustomTaskDirect(taskId, link) {
        if(link) tg.openLink(link);
        setTimeout(async () => {
            const res = await apiCall('claim_task', { taskId });
            if(res && !res.error) showToast('Uğurlu', `Tapşırıq bitdi! +${res.gain} XP!`, 'success');
        }, 2000);
    }
    ...
    async function claimSponsorTask() {
        ...
        setTimeout(async () => { const res = await apiCall('claim_task', { taskId: 'sponsor_azx' }); if(res && !res.error) showToast('Sponsor Task', `You earned +${res.gain} XP!`, 'success'); }, 1500);
    }
    async function claimTask(taskId, reward) {
        if(tg.HapticFeedback) tg.HapticFeedback.impactOccurred('medium');
        const res = await apiCall('claim_task', { taskId, reward });
        if(res && !res.error) showToast('Task Completed!', `You earned +${res.gain} XP!`, 'success');
    }
    ...
    async function watchAd() {
      ...
      for(let i = 0; i < blockIds.length; i++) {
        ...
        try {
          ...
          // Silent catch to allow looping to next SDK
          console.warn(`Adsgram API (${currentId}) xətası. Növbəti yoxlanılır...`);
        }
      }
    ...
    // Background polling for real-time updates without refreshing
    setInterval(async () => {
        if(appState.user && appState.user.tgId && !document.hidden) {
            await apiCall('sync', {}, true);
        }
    }, 10000);
    ...
    async function submitAdminAuth() {
        ...
        adminToken = code;
        const res = await apiCall('admin_dashboard', { adminCode: code }, true);
        if(res && !res.error) {
            closeAdminAuth();
            switchTab('admin');
            ...
            // Populate Settings
            document.getElementById('admin-ad-sdk').value = (res.adminSettings.adBlockIds || []).join(', ');
            document.getElementById('admin-maintenance').checked = res.adminSettings.maintenance || false;
            document.getElementById('admin-uid').value = res.adminSettings.adminUid || '5461064199';
            document.getElementById('admin-reset-time').value = res.adminSettings.resetTime || '03:00';
            document.getElementById('admin-xp-mult').value = res.adminSettings.xpMultiplier || 1;

            adminUsers = res.all_users;
            adminWithdrawals = res.all_withdrawals;
            adminCustomTasks = res.custom_tasks;
            adminSubmissions = res.submissions;
            ...
            <div class="glass-card p-3 rounded-lg border border-slate-700 space-y-2">
                <div class="flex justify-between items-center border-b border-slate-700/50 pb-2">
                    <div>
                        ...
                        <p class="text-[9px] text-blue-400 font-mono">UID: ${u.tgId}</p>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-2 text-[9px] text-slate-400 mb-1">
                    <p>Günlük Reklam: <strong class="text-white">${u.adsWatchedToday}</strong></p>
                    <p>Ümumi Reklam: <strong class="text-white">${u.totalAdsWatched}</strong></p>
                    <p>Referallar: <strong class="text-white">${u.refCount || 0}</strong></p>
                    <p>Bitən Tapşırıqlar: <strong class="text-white">${u.tasksCompleted}</strong></p>
                    <p>Xərclənən XP: <strong class="text-crypto-bronze">${u.xpSpent}</strong></p>
                    <p>Son Aktivlik: <strong class="text-blue-300">${u.lastActiveAz}</strong></p>
                </div>
                ...
                <div class="flex gap-2 items-center bg-[#050511] p-2 rounded">
                    ...
                    <button onclick="adminSaveUser('${u.tgId}')" class="bg-blue-600/20 text-blue-400 px-3 py-1 rounded text-[10px] uppercase font-black ml-auto">Yadda Saxla</button>
                </div>
                <div class="flex gap-2">
                    <button onclick="adminActionUser('${u.tgId}', 'reset_ads')" class="flex-1 bg-amber-500/20 text-amber-500 py-1.5 rounded text-[9px] uppercase font-black">Reklamı Sıfırla</button>
                    <button onclick="adminActionUser('${u.tgId}', '${u.banned ? 'unban' : 'ban'}')" class="flex-1 ${u.banned ? 'bg-emerald-500/20 text-emerald-500' : 'bg-red-500/20 text-red-500'} py-1.5 rounded text-[9px] uppercase font-black">${u.banned ? 'Bandan Çıxar' : 'Ban Et'}</button>
                </div>
            </div>
        `).join('');
    }
    ...
    async function adminActionUser(targetUid, userAction, extras = {}) {
        const res = await apiCall('admin_action_user', { adminCode: adminToken, targetUid, userAction, ...extras }, true);
        if(res && !res.error) {
            showToast('Uğurlu', res.message, 'success');
            submitAdminAuth(); // Refresh data
        } else if (res.error) showToast('Xəta', res.error, 'error');
    }

    function renderAdminWithdrawals() {
        const pending = adminWithdrawals.filter(w => w.status === 'Pending');
        if(pending.length === 0) {
            document.getElementById('admin-withdrawal-list').innerHTML = '<p class="text-center text-xs text-slate-500 py-4">Gözləyən çıxarış yoxdur</p>';
            return;
        }
        document.getElementById('admin-withdrawal-list').innerHTML = pending.map(w => `
            <div class="glass-card p-3 rounded-lg border border-slate-700">
                <div class="flex justify-between items-center mb-1">
                    <div>
                        <p class="text-xs font-black text-white">ID: ${w.user_id} <span class="text-[9px] font-mono text-slate-400">@${w.username}</span></p>
                        <p class="text-[9px] text-slate-400">${w.date}</p>
                    </div>
                    <p class="text-sm font-black text-emerald-400">$${w.amount.toFixed(2)}</p>
                </div>
                
                <div class="flex gap-2 text-[9px] text-slate-300 mb-2 border-b border-slate-700/50 pb-1">
                    <p>Mövcud Balans: <strong class="text-emerald-400">$${w.user_usd.toFixed(2)}</strong></p>
                    <p>Xərclənən XP: <strong class="text-crypto-bronze">${w.user_xp_spent}</strong></p>
                </div>

                <div class="bg-[#050511] p-1.5 rounded mb-2 overflow-hidden text-ellipsis border border-slate-800">
                    <p class="text-[10px] text-blue-400 font-mono tracking-wider">${w.address}</p>
                </div>
                <div class="flex gap-2">
                    <button onclick="adminWithdrawAction('${w.user_id}', ${w.idx}, 'approve')" class="flex-1 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-500 py-1.5 rounded text-[10px] uppercase font-black transition">Təsdiqlə</button>
                    <button onclick="adminWithdrawAction('${w.user_id}', ${w.idx}, 'reject')" class="flex-1 bg-red-500/20 hover:bg-red-500/30 text-red-500 py-1.5 rounded text-[10px] uppercase font-black transition">Rədd Et</button>
                </div>
            </div>
        `).join('');
    }
    ...
    async function adminWithdrawAction(targetUid, idx, action) {
        if(confirm(`Bu çıxarışı ${action === 'approve' ? 'TƏSDİQLƏMƏK' : 'RƏDD ETMƏK'} istədiyinizə əminsiniz?`)) {
            const res = await apiCall('admin_action_withdraw', { adminCode: adminToken, targetUid, idx, withdrawAction: action }, true);
            if(res && !res.error) {
                showToast('Uğurlu', res.message, 'success');
                submitAdminAuth();
            }
    ...
    function renderAdminTasks() {
        ...
        const tasks = Object.values(adminCustomTasks);
        if(tasks.length === 0) {
            container.innerHTML = `<p class="text-center text-xs text-slate-500 py-4">Heç bir tapşırıq yoxdur</p>`;
            return;
        }
            ...
            <div class="glass-card p-3 rounded-lg border border-slate-700">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <p class="text-[9px] text-slate-400 mb-1">${t.desc}</p>
                    </div>
                    <span class="text-[8px] px-2 py-1 rounded ${t.active ? 'bg-emerald-500/20 text-emerald-500' : 'bg-red-500/20 text-red-500'} font-black uppercase">${t.active ? 'Aktiv' : 'Passiv'}</span>
                </div>
                <p class="text-[9px] text-blue-400 break-all mb-1">${t.link}</p>
                <div class="flex justify-between items-center text-[10px]">
                    <span class="text-crypto-glow font-black">+${t.reward} XP</span>
                    <span class="text-amber-400">${t.requireInfo ? `Məlumat tələb edilir (${t.infoLabel})` : 'Məlumat istənmir'}</span>
                </div>
                <div class="flex gap-2 mt-2">
                    <button onclick="adminToggleTask('${t.id}', ${!t.active})" class="flex-1 py-1.5 rounded text-[9px] font-black uppercase bg-slate-800 text-white hover:bg-slate-700 transition">${t.active ? 'Deaktiv Et' : 'Aktivləşdir'}</button>
                </div>
            </div>
        `).join('');
    }
    ...
    async function adminSaveTask() {
        ...
        const reward = document.getElementById('nt-reward').value;
        const requireInfo = document.getElementById('nt-reqInfo').checked;
        const infoLabel = document.getElementById('nt-infoLabel').value;
        
        if(!title || !reward) return showToast('Xəta', 'Başlıq və mükafat mütləqdir', 'error');

        const res = await apiCall('admin_save_task', { adminCode: adminToken, title, desc, link, reward, requireInfo, infoLabel, active: true }, true);
        if(res && !res.error) {
            showToast('Uğurlu', res.message, 'success');
            submitAdminAuth();
        }
    ...
    async function adminToggleTask(taskId, active) {
        const t = adminCustomTasks[taskId];
        const res = await apiCall('admin_save_task', { adminCode: adminToken, taskId: t.id, title: t.title, desc: t.desc, link: t.link, reward: t.reward, requireInfo: t.requireInfo, infoLabel: t.infoLabel, active: active }, true);
        if(res && !res.error) { showToast('Uğurlu', 'Vəziyyət dəyişdirildi', 'success'); submitAdminAuth(); }
    }
    ...
    function renderAdminSubmissions() {
        ...
        if(pending.length === 0) {
            container.innerHTML = `<p class="text-center text-xs text-slate-500 py-4">Təsdiq gözləyən tapşırıq yoxdur</p>`;
            return;
        }

        container.innerHTML = pending.map(s => `
            <div class="glass-card p-3 rounded-lg border border-amber-500/30">
                <p class="text-[10px] font-black text-amber-400 mb-1">Tapşırıq: ${s.taskTitle}</p>
                <div class="flex justify-between items-center border-b border-slate-700/50 pb-1 mb-1">
                    <div>
                        <p class="text-xs font-black text-white">${s.name} <span class="text-[9px] text-slate-400 font-mono">@${s.username}</span></p>
                        <p class="text-[9px] text-blue-400 font-mono">UID: ${s.uid}</p>
                    </div>
                    <p class="text-[8px] text-slate-500">${s.date}</p>
                </div>
                <div class="bg-[#050511] p-2 rounded mb-2 border border-slate-800">
                    <p class="text-[9px] text-slate-500 uppercase tracking-widest mb-0.5">Daxil edilən məlumat:</p>
                    <p class="text-sm font-mono text-white">${s.infoSubmitted}</p>
                </div>
                <div class="flex gap-2">
                    <button onclick="adminActionSubmission('${s.subId}', 'approve')" class="flex-1 bg-emerald-500/20 text-emerald-500 hover:bg-emerald-500/30 py-1.5 rounded text-[10px] uppercase font-black transition">Təsdiqlə</button>
                    <button onclick="adminActionSubmission('${s.subId}', 'reject')" class="flex-1 bg-red-500/20 text-red-500 hover:bg-red-500/30 py-1.5 rounded text-[10px] uppercase font-black transition">Rədd Et</button>
                </div>
            </div>
        `).join('');
    }
    ...
    async function adminActionSubmission(subId, subAction) {
        const res = await apiCall('admin_action_submission', { adminCode: adminToken, subId, subAction }, true);
        if(res && !res.error) {
            showToast('Uğurlu', res.message, 'success');
            submitAdminAuth();
        }
    ...
    async function saveAdminSettings() {
        ...
        const maintenance = document.getElementById('admin-maintenance').checked;
        const adminUid = document.getElementById('admin-uid').value;
        const resetTime = document.getElementById('admin-reset-time').value;
        const xpMultiplier = document.getElementById('admin-xp-mult').value;
        
        const res = await apiCall('admin_update_settings', { adminCode: adminToken, blockIds, maintenance, adminUid, resetTime, xpMultiplier }, true);
        if(res && !res.error) {
            showToast('Uğurlu', res.message, 'success');
    ...
    async function adminResetAllAds() {
        if(confirm('Bütün istifadəçilərin günlük reklam limiti sıfırlanacaq. Davam edilsin?')) {
            const res = await apiCall('admin_reset_all_ads', { adminCode: adminToken }, true);
            if(res && !res.error) { showToast('Uğurlu', res.message, 'success'); submitAdminAuth(); }
        }
    }
    
    async function adminResetBot() {
        if(confirm('DİQQƏT! Botdakı bütün istifadəçi məlumatları, balanslar, referallar SİLİNƏCƏK. Əminsiniz?')) {
            const res = await apiCall('admin_reset_bot', { adminCode: adminToken }, true);
            if(res && !res.error) { showToast('Uğurlu', res.message, 'success'); submitAdminAuth(); }
        }
    }
